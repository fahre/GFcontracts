...
interface Ibuffer {
    function aboutGame(uint256 game)
        external
        view
        returns (
            address owner,
    ...
    function aboutUser(address user)
        external
        view
        returns (
            uint256 character,
...
contract buffer is Ibuffer {
        ...
        )
    {
        ...
        if (game != 0) {
            gameIndex = _gameIndex[msg.sender];
        } else {
            gameIndex = 0;
        }
    }
    ...
    function joinGame(uint256 game) external override {
        ...
        require(
            game < _games &&
                ...
                _NFTowner[msg.sender] != 0 &&
                _game[msg.sender] == 0 &&
                block.number < _endingBlock[game]
        );
        if (_participationFee[game] > 0) {
            NGL.transferFrom(msg.sender, _fieldOwner[game], _participationFee[game]);
        }
        if (_participant[game][index] != address(0)) {
            _participant[game][index] = msg.sender;
            _gameIndex[msg.sender] = index;
        } else {
            for (uint256 t; t < 20; ++t) {
                if (_participant[game][t] == address(0)) {
                    _participant[game][t] = msg.sender;
                    _gameIndex[msg.sender] = t;
                    break;
                }
            }
        }
