...
interface INFT {
    ...
    function forgeResult(uint256 firstNFT, uint256 secondNFT)
        external
        view
        returns (
            uint256 category,
    ...
    function mintNFT(
        ...
        uint256 category,
        uint256 level,
        string calldata more
    ) external returns (uint256);

    function burnNFT(uint256 ID) external;

    function setlevelsPrices(
        uint256 ID,
        uint256 level1,
        uint256 level2,
        uint256 level3,
    ...
    function createForgeEquation(
        uint256 category1,
        uint256 category2,
        uint256 resultingCategory
    ) external;
...
contract NFT is INFT {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) /*category*/ /*category*/ /*resulting category*/
        private _forgeEquation;
    mapping(uint256 => mapping(uint256 => string)) /*category*/ /*category*/ /*name of resulting category*/
        private _forgeName;
    mapping(uint256 => mapping(uint256 => string)) /*category*/ /*category*/ /*symbol of resulting category*/
        private _forgeSymbol;
    ...
    function transferFrom(
        ...
        if (_allowance[from][msg.sender][tokenId]) {
            _allowance[from][msg.sender][tokenId] = false;
        }
        _balances[from] -= 1;
        _balances[to] += 1;
        _owner[tokenId] = to;
    ...
    function forge(uint256 tokenID, uint256 tokenID1) external override returns (uint256) {
        ...
        uint256 nft = MintNFT(
            _forgeName[tokenID][tokenID1],
            _forgeSymbol[tokenID][tokenID1],
            msg.sender,
            _forgeEquation[tokenID][tokenID1],
            0,
            ""
        );
        _boxed1[nft] = tokenID;
